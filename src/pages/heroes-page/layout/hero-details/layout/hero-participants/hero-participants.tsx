...
export const HeroParticipants: FC = () => {
  const { id } = useParams()
  ...
  return participants?.map((participantEl) => {
    return {
      ...
      cells: [
        ...
        <p key='2'>{`${participantEl.surname} ${participantEl.firstname}`}</p>,
        <p key='3'>{participantEl.region_name}</p>,
        <p key='4'>{'Прозвище'}</p>,
        <p className={styles.ageCell} key='5'>{participantEl.age}</p>,
        <p key='6'>{participantEl.group_name}</p>,
        <p key='7'>{'3 события'}</p>,
        <p key='8'>
